...
void PackedDB::load_fasta_db(const char* dbname, const int max_seq_size, std::vector<std::string>& names, std::unordered_map<std::string, std::array<int, 2>>& name2id)
{
	DynamicTimer dtimer(__func__);
	FastaReader freader(dbname);
	Sequence seq;
	while(1)
	{
		...
		if(size == -1) break;
		add_one_seq(seq);
		std::string s(seq.header().begin(), seq.header().size());
		name2id[s] = {(int)names.size(), (int)seq.sequence().size()};
		names.push_back(s);
	}
}
