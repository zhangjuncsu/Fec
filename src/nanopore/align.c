...
static int64 OvlIOSize = sizeof(Overlaps) - sizeof(void *);

int Read_Overlap(FILE *input, Overlaps *ovl)
{ if (fread( ((char *) ovl) + PtrSize, OvlIOSize, 1, input) != 1)
    return (1);
  return (0);
}

int Read_Trace(FILE *input, Overlaps *ovl, int tbytes)
...
int Write_Overlap(FILE *output, Overlaps *ovl, int tbytes)
...
void Compress_TraceTo8(Overlaps *ovl)
...
void Decompress_TraceTo16(Overlaps *ovl)
{ uint16 *t16 = (uint16 *) ovl->path.trace;
  uint8  *t8  = (uint8  *) ovl->path.trace;
  int     j;
...
void Print_Overlap(FILE *output, Overlaps *ovl, int tbytes, int indent)
{ int     i;
...
int Check_Trace_Points(Overlaps *ovl, int tspace, int verbose, char *fname)
{ int i, p, q; 
...
void
Print_Align_String(FILE* file, kstring_t* qaln, kstring_t* taln)
{
	assert(kstr_size(*qaln) == kstr_size(*taln));
	int n = kstr_size(*qaln);
	if (n == 0) return;
	for (int i = 0; i < n; ++i) {
		char c = kstr_A(*qaln, i);
		fprintf(file, "%c", c);
	}
	fprintf(file, "\n");
	
	for (int i = 0; i < n; ++i) {
		char c = kstr_A(*taln, i);
		fprintf(file, "%c", c);
	}
	fprintf(file, "\n");
	...
	int m = 0;
	for (int i = 0; i < n; ++i) {
		char qc = kstr_A(*qaln, i);
		char tc = kstr_A(*taln, i);
		if (qc == tc) ++m;
	}
	double ident_perc = 100.0 * m / n;
	fprintf(file, "ident_perc = %f\n", ident_perc);
}
