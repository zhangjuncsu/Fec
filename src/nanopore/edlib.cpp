...
static int obtainAlignment(
        ...
        unsigned char** alignment, int* alignmentLength, void* km);
...
extern "C" EdlibAlignResult edlibAlign(const char* const queryOriginal, const int queryLength,
                                       ...
                                       const EdlibAlignConfig config, void* km) {
    ...
    Word* Peq = buildPeq((int) alphabet.size(), query, queryLength, equalityDefinition, km);
    /*-------------------------------------------------------*/
    ...
    if (result.editDistance >= 0) {  // If there is solution.
        ...
        if (config.mode == EDLIB_MODE_NW) {
            result.endLocations = (int *) kmalloc(km, sizeof(int) * 1);
        ...
        if (config.task == EDLIB_TASK_LOC || config.task == EDLIB_TASK_PATH) {
            result.startLocations = (int*) kmalloc(km, result.numLocations * sizeof(int));
            if (config.mode == EDLIB_MODE_HW) {  // If HW, I need to calculate start locations.
                const unsigned char* rTarget = createReverseCopy(km, target, targetLength);
                const unsigned char* rQuery  = createReverseCopy(km, query, queryLength);
                // Peq for reversed query.
                Word* rPeq = buildPeq((int) alphabet.size(), rQuery, queryLength, equalityDefinition, km);
                for (int i = 0; i < result.numLocations; i++) {
                    ...
                    if (endLocation == -1) {
                        ...
                        myersCalcEditDistanceSemiGlobal(
                                ...
                                &bestScoreSHW, &positionsSHW, &numPositionsSHW, km);
        ...
        if (config.task == EDLIB_TASK_PATH) {
            ...
            const unsigned char* rAlnTarget = createReverseCopy(km, alnTarget, alnTargetLength);
            const unsigned char* rQuery  = createReverseCopy(km, query, queryLength);
            obtainAlignment(query, rQuery, queryLength,
                            ...
                            &(result.alignment), &(result.alignmentLength), km);
...
static inline Word* buildPeq(const int alphabetLength,
	...
	Word* Peq = (Word*)kmalloc(km, sizeof(Word) * (alphabetLength + 1) * maxNumBlocks);
...
static inline unsigned char* createReverseCopy(void* km, const unsigned char* const seq, const int length) {
	unsigned char* rSeq = (unsigned char*)kmalloc(km, length);
 ...
 */
static inline void getBlockCellValues(const Block block, int scores[]) {
...
static int myersCalcEditDistanceSemiGlobal(
    ...
    *numPositions_ = 0;
	...
	Block* blocks = (Block*)kmalloc(km, sizeof(Block) * maxNumBlocks);
    ...
    for (int c = 0; c < targetLength; c++) { // for each column
        ...
        if (lastBlock < firstBlock) {
            *bestScore_ = bestScore;
            if (bestScore != -1) {
				*positions_ = (int*) kmalloc(km, sizeof(int) * positions.size());
...
static int obtainAlignment(
        ...
        unsigned char** const alignment, int* const alignmentLength, void* km) {
    ...
    if (alignmentDataSize < 1024 * 1024) {
        ...
        AlignmentData* alignData = NULL;
        Word* Peq = buildPeq(alphabetLength, query, queryLength, equalityDefinition, km);
        myersCalcEditDistanceNW(Peq, W, maxNumBlocks,
                                queryLength,
                                target, targetLength,
                                bestScore,
                                &score_, &endLocation_, true, &alignData, -1, km);
...
static int obtainAlignmentHirschberg(
    ...
    Word* Peq = buildPeq(alphabetLength, query, queryLength, equalityDefinition, km);
    Word* rPeq = buildPeq(alphabetLength, rQuery, queryLength, equalityDefinition, km);
    ...
    int leftHalfCalcStatus = myersCalcEditDistanceNW(
            ...
            &score_, &endLocation_, false, &alignDataLeftHalf, leftHalfWidth - 1, km);
    ...
    int rightHalfCalcStatus = myersCalcEditDistanceNW(
            ...
            &score_, &endLocation_, false, &alignDataRightHalf, rightHalfWidth - 1, km);
	...
	int* scoresRight = (int*)kmalloc(km, sizeof(int) * scoresRightLength);
    ...
    int ulStatusCode = obtainAlignment(query, rQuery + lrHeight, ulHeight,
                                       ...
                                       &ulAlignment, &ulAlignmentLength, km);
    ...
    int lrStatusCode = obtainAlignment(query + ulHeight, rQuery, lrHeight,
                                       ...
                                       &lrAlignment, &lrAlignmentLength, km);
...
static string transformSequences(const char* const queryOriginal, const int queryLength,
    ...
    *queryTransformed = (unsigned char *) kmalloc(km, sizeof(unsigned char) * queryLength);
    *targetTransformed = (unsigned char *) kmalloc(km, sizeof(unsigned char) * targetLength);
...
extern "C" void edlibFreeAlignResult(EdlibAlignResult result, void* km) {
...
void edlibAlignmentToStrings(const unsigned char* alignment, 
							 int alignmentLength, 
							 int tgtStart, int tgtEnd, 
							 int qryStart, int qryEnd, 
							 const char *tgt, 
							 const char *qry, char *tgt_aln_str, 
							 char *qry_aln_str) {
