...
define ADD_TARGET_RULE
    ...
    ifeq "$$(suffix ${1})" ".jar"
	    ...
	    $$(strip $${${1}_LINKER} -o $$@ $${LDFLAGS} $${${1}_LDFLAGS} \
	        $${${1}_OBJS} $${LDLIBS} $${${1}_LDLIBS}) $${LDFLAGS}
	    $${${1}_POSTMAKE}
    endif
    endif
endef
...
ifeq ($(BUILDDEBUG), 1)
	CXXFLAGS = -D_GLIBCXX_PARALLEL -pthread -O0 -Wall -g -fopenmp -std=c++11
	CFLAGS = -D_GLIBCXX_PARALLEL -pthread -O0 -Wall -g -fopenmp -std=gnu99
else
	CXXFLAGS = -D_GLIBCXX_PARALLEL -pthread -O3 -Wall -std=c++11
	CFLAGS = -D_GLIBCXX_PARALLEL -pthread -O3 -Wall -std=gnu99
endif

LDFLAGS = -pthread -lm -fopenmp -lz -lstdc++
